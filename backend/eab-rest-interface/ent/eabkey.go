...
func (*EABKey) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case eabkey.FieldID:
			values[i] = new(sql.NullInt64)
		case eabkey.FieldUser, eabkey.FieldEabKey, eabkey.FieldComment:
...
func (ek *EABKey) assignValues(columns []string, values []any) error {
...
func (ek *EABKey) Unwrap() *EABKey {
	_tx, ok := ek.config.driver.(*txDriver)
	if !ok {
		panic("ent: EABKey is not a transactional entity")
	}
	ek.config.driver = _tx.drv
	return ek
}
...
func (ek *EABKey) String() string {
	...
	builder.WriteString(ek.Comment)
	builder.WriteByte(')')
