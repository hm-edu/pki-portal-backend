...
func (h *Handler) GetExternalAccountKeys(c echo.Context) error {
	...
	keys, _, err := database.DB.NoSQL.GetExternalAccountKeys(ctx, "acme", "", -1)
	if err != nil {
...
func (h *Handler) CreateNewKey(c echo.Context) error {
	...
	defer span.End()
	h.logger.Info("Requesting external account keys")
	key, err := database.DB.NoSQL.CreateExternalAccountKey(ctx, "acme", "")
...
func (h *Handler) DeleteKey(c echo.Context) error {
	...
	err = database.DB.NoSQL.DeleteExternalAccountKey(ctx, "acme", key)
	if err != nil {
