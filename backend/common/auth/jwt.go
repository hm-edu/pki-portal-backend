		...
		return func(c echo.Context) error {
			...
			for _, extractor := range extractors {
				...
				for _, auth := range auths {
					...
					sub, err := SubFromRequest(token)
					if err != nil {
						lastTokenErr = err
						continue
					}
					logger := c.Request().Context().Value(logging.LoggingContextKey).(*zap.Logger)
					logger = logger.With(zap.String("user", sub))
					c.SetRequest(c.Request().WithContext(context.WithValue(c.Request().Context(), logging.LoggingContextKey, logger)))
