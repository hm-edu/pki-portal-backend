...
import (
	...
	"github.com/hm-edu/pki-handler/pkg/helper"
...
func (s *Syncer) SyncCertificates() {
	...
	for {
		...
		for _, item := range details {
			...
			creator := s.Db.Certificate.Create().SetCommonName(item.CommonName).SetSslId(item.SslID).SetNotAfter(item.Expires.Time).SetSerial(helper.NormalizeSerial(item.SerialNumber)).AddDomainIDs(ids...)

			if item.Requested != nil {
				creator.SetNotBefore(item.Requested.Time)
			}
			creator.SetStatus(certificate.Status(item.Status))
			err = creator.OnConflict().UpdateNewValues().Exec(ctx)
			if err != nil {
